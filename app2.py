...
def generate_illuminated_catalogs_updated(bezel_label, operator_label, lens_color_label, contact_block_label, light_unit_label):
    bezel_codes = get_code(bezel_df, bezel_label)
    operator_rows = operator_df if operator_label == "Any" else operator_df[operator_df["label"] == operator_label]
...
with tabs[1]:
    ...
    lu_voltage_choice = st.selectbox("Light Unit Voltage", ["Any"] + light_df["lu_voltage"].unique().tolist(), key="luv")

    if st.button("Generate Illuminated Catalog Numbers"):
        st.session_state["illuminated_df"] = generate_illuminated_catalogs_updated(
            bezel_choice, operator_choice, lens_choice, contact_block_choice, lu_voltage_choice
        )
